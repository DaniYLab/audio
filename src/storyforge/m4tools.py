...
def _pick_thumbnail_scene(story: Story, settings: Settings) -> StoryBeat | None:
    """M4-A3: use `thumbnail_scene: N|auto` from config if present, else the
    first scene; no rubric rerun — best-effort deterministic selection."""
    scenes = story.scenes
    if not scenes:
        return None
    selected = scenes[0]
...
def auto_thumbnail(settings: Settings, store: ArtifactStore) -> Path | None:
    """Best-effort thumbnail: overlay title on the first scene's image.

    Never raises — pipeline continues if anything fails (A3 AC4).
    """
    try:
        story = store.read_model(store.story_path(), Story)
        beat = _pick_thumbnail_scene(story, settings)
        if beat is None or not story.scenes:
            return None
